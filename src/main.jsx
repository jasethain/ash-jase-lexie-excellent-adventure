...
// Lightweight kana-to-romaji helper for translation display.
// It handles hiragana/katakana well. Kanji cannot be perfectly romanised without a dictionary,
// so kanji phrases still keep useful romaji from the phrasebook when available.
const KANA_ROMAJI = {
...
function toRomaji(input = "") {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (one === "っ" || one === "ッ") {
      ...
      const next = KANA_ROMAJI[nextTwo] || KANA_ROMAJI[nextOne] || "";
      out += next ? next[0] : "";
      continue;
    }
    if (one === "ー") { out += "-"; continue; }
    if (KANA_ROMAJI[one]) { out += KANA_ROMAJI[one]; continue; }
    if (/[、。！？?!.]/.test(one)) { out += one + " "; continue; }
    if (/\s/.test(one)) { out += " "; continue; }
    out += one;
  }
  return out.replace(/\s+/g, " ").trim().replace(/\b\w/g, c => c.toUpperCase());
}

function phraseRomaji(english, japanese, fallback = "") {
  const key = String(english || "").toLowerCase().trim().replace(/[?!.。！？]+$/g, "");
  if (typeof OFFLINE_TRANSLATIONS !== "undefined" && OFFLINE_TRANSLATIONS[key]?.romaji) return OFFLINE_TRANSLATIONS[key].romaji;
  if (fallback) return fallback;
  return toRomaji(japanese);
}
